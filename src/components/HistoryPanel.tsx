    ...
    <div className="history-modal-overlay" onClick={onClose}>
      <div className="history-panel glass-panel cursor-default" onClick={e => e.stopPropagation()}>
        <div className="history-header">
          <h2>🍔 마이 런치 로그</h2>
          <button className="close-btn" onClick={onClose} style={{color: 'var(--text-dark)'}}>✕</button>
        </div>
        
        <p className="history-desc" style={{ color: 'var(--text-muted)' }}>
          최근 3일 내 달린 점심 메뉴는 룰렛에서 센스 있게 필터링해 드려요! 😎 
        </p>
            ...
            {history.map(item => (
              <li key={item.id} className="history-item">
                <div className="history-info">
                  ...
                  <span style={{ fontSize: '0.85rem' }}>📍 {getRestaurantName(item.restaurantId)}</span>
        ...
        {history.length > 0 && (
          <div className="history-footer" style={{ marginTop: '1rem', display: 'flex', justifyContent: 'flex-end' }}>
            <button className="clear-btn" onClick={onClear} style={{ background: 'var(--border-strong)', color: 'var(--text-dark)' }}>
              🧹 전체 로그 리셋
            </button>
          </div>
        )}
